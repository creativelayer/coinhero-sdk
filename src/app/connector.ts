...
import { type Connector, type CreateConnectorFn, createConnector } from '@wagmi/core'
...
export function coinHeroConnector(sdk?: CoinHeroSDK): CreateConnectorFn {
  ...
  return createConnector((config) => ({
    ...
    async connect({ chainId } = {}) {
      ...
      if (!targetChainId) {
        const state = (await config.storage?.getItem('state')) as { chainId?: number } | undefined
      ...
      if (!disconnectHandler) {
        ...
        provider.on('disconnect', disconnectHandler as any)
      }
    ...
    async disconnect() {
      ...
      if (disconnectHandler) {
        provider.removeListener('disconnect', disconnectHandler as any)
    ...
    async getChainId() {
      const hexChainId = (await _sdk.provider.request({ method: 'eth_chainId' })) as string
